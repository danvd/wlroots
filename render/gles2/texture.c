...
static void gles2_client_egl_stream_destroy(struct wl_listener *listener, void *data) {
	...
	glDeleteTextures(1, &client_stream->texture->tex);
	wlr_egl_restore_context(&prev_ctx);
	wl_list_remove(&client_stream->texture->link);
	wl_list_remove(&client_stream->texture->buffer_destroy.link);
...
struct wlr_buffer *gles2_buffer_from_wl_eglstream(struct wlr_renderer *wlr_renderer,
	...
	if (ok) {
		...
		ok = egl->procs.eglStreamConsumerAcquireAttribNV(egl->display, stream, NULL) == EGL_TRUE;
	}
	...
	struct wlr_gles2_texture *texture = gles2_texture_create(renderer, width, height);
	if (texture == NULL) {
	...
	texture->tex = tex;
	attached_stream->texture = texture;
...
static struct wlr_texture *gles2_texture_from_wl_eglstream(struct wlr_buffer *buffer) {
	...
	return &client_stream->texture->wlr_texture;
}
